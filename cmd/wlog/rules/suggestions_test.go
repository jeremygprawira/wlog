...
func TestRules_UseCatalog(t *testing.T) {
	...
	fired := checkByID(t, rules.Evaluate(pkgs[0], pointIn(t, points, "handleLiteral"), rules.Config{}), rules.RuleUseCatalog)
	if fired.Pass {
		t.Error("handleLiteral passed: APP_NOT_FOUND is in the registry")
	}

	other := checkByID(t, rules.Evaluate(pkgs[0], pointIn(t, points, "handleOtherLiteral"), rules.Config{}), rules.RuleUseCatalog)
	if !other.Pass {
	...
	quiet := checkByID(t, rules.Evaluate(quietPkgs[0], quietPoints[0], rules.Config{}), rules.RuleUseCatalog)
	if !quiet.Pass {
...
func TestRules_AuditCoverage(t *testing.T) {
	...
	write := checkByID(t, rules.Evaluate(pkgs[0], pointIn(t, points, "handleWriteNoAudit"), rules.Config{}), rules.RuleAuditCoverage)
	if write.Pass {
		t.Error("handleWriteNoAudit passed: a write route with no audit record")
	}

	read := checkByID(t, rules.Evaluate(pkgs[0], pointIn(t, points, "handleReadNoAudit"), rules.Config{}), rules.RuleAuditCoverage)
	if !read.Pass {
