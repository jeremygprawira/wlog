...
func TestRules_SwallowedError(t *testing.T) {
	...
	discarded := checkByID(t, rules.Evaluate(pkgs[0], pointIn(t, points, "handleDiscard"), rules.Config{}), rules.RuleSwallowedError)
	if discarded.Pass {
		t.Error("handleDiscard passed: the error is discarded with no report")
	}

	empty := checkByID(t, rules.Evaluate(pkgs[0], pointIn(t, points, "handleEmptyBranch"), rules.Config{}), rules.RuleSwallowedError)
	if empty.Pass {
		t.Error("handleEmptyBranch passed: the error branch does nothing")
	}

	good := checkByID(t, rules.Evaluate(pkgs[0], pointIn(t, points, "handleGood"), rules.Config{}), rules.RuleSwallowedError)
	if !good.Pass {
		t.Errorf("handleGood failed: %s", good.Detail)
	}

	unresolvable := checkByID(t, rules.Evaluate(pkgs[0], pointIn(t, points, "handleUnresolvable"), rules.Config{}), rules.RuleSwallowedError)
