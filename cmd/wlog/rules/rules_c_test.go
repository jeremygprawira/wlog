...
func TestRules_ErrorGuidance(t *testing.T) {
	...
	bad := checkByID(t, rules.Evaluate(pkgs[0], pointIn(t, points, "handleBad"), rules.Config{}), rules.RuleErrorGuidance)
	if bad.Pass {
		t.Error("handleBad passed: errors.New carries no guidance")
	}

	good := checkByID(t, rules.Evaluate(pkgs[0], pointIn(t, points, "handleGood"), rules.Config{}), rules.RuleErrorGuidance)
	if !good.Pass {
		t.Errorf("handleGood failed: %s", good.Detail)
	}

	none := checkByID(t, rules.Evaluate(pkgs[0], pointIn(t, points, "handleNone"), rules.Config{}), rules.RuleErrorGuidance)
	if !none.Pass {
