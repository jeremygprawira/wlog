...
func Rate(level wlog.Level, percent int) Option // head: 0-100, default 100 for every level
...
`Keep` evaluates in this order: (1) any tail condition (`KeepStatus`/`KeepDuration`/`KeepPath`/
...
a random draw against `Rate(event's level)` (default 100, i.e. always kept). `wlog.Logger`
...
`KeepErrorsAndSlow(slow, healthyRate)` is sugar for `New(KeepDuration(slow), Rate(wlog.LevelInfo,
healthyRate), Rate(wlog.LevelDebug, healthyRate))` — errors are always kept per rule (2) above.
...
2. `Rate(wlog.LevelInfo, 0)` drops every info event; `Rate(wlog.LevelInfo, 100)` keeps all;
   `Rate(wlog.LevelError, 0)` still keeps error events (rule 2 overrides head sampling for
   errors).
