...
| `job-cron` | `cron.WithChain(Wrap(name, spec))`, placed inside `SkipIfStillRunning`. `Job(name, fn func(ctx) error)` gives a context and an error. `work.Ticker` covers a plain `time.Ticker` loop | system `cron`, name, schedule. `Stop()`'s context is the flush point | cron v3.0.1, Go 1.21 |
