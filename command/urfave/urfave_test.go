...
// workFactory drives the real Run path with one command, so a change that breaks the adapter
// fails the suite.
...
func (workFactory) Process(log *wlog.Logger, unit work.Unit, handler func(context.Context) error) (err error) {
	...
	cmd := &cli.Command{Name: "wlog", Commands: []*cli.Command{{
		Name: "map",
	...
	_ = Run(context.Background(), log, cmd, []string{"wlog", "map"})
