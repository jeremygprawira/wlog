...
// workFactory drives the real Handler path with a fake claim, so a change that breaks the
// adapter fails the suite.
type workFactory struct{}

// Declare names the one kind a sarama consumer produces. Kafka reports no delivery count.
...
// Process runs one unit of work through the claim loop. The suite expects no panic from
// Process, so the panic of the handler, which the loop raises again, comes back as an error.
func (workFactory) Process(log *wlog.Logger, unit work.Unit, handler func(context.Context) error) (err error) {
	...
	claim := &fakeClaim{
		topic: destination, partition: 1, highWaterMark: 10,
		messages: []*sarama.ConsumerMessage{{
			Topic: destination, Partition: 1, Offset: 7, Timestamp: unit.StartedAt,
		}},
	}
	h := Handler(log, func(ctx context.Context, _ *sarama.ConsumerMessage) error {
		return handler(ctx)
	})
	return h.ConsumeClaim(newFakeSession(), claim)
}
